...
Usage:
  # Test default question (asks for a search)
  python scripts/test_vllm_tool_call.py --base_url http://localhost:8001/v1 --model openai/gpt-oss-20b

  # Custom question
  python scripts/test_vllm_tool_call.py --base_url http://localhost:8001/v1 --model openai/gpt-oss-20b \
    --question "What is the capital of France? Use search if needed."
"""
import argparse
import asyncio
import json
...
def _build_prompt(model_name: str, question: str) -> str:
...
async def test_tool_call(base_url: str, model: str, question: str) -> bool:
    base_url = base_url.rstrip("/")
    prompt = _build_prompt(model, question)

    # Resolve model name for API (vLLM may expose a different id)
    api_model = model
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            r = await client.get(f"{base_url}/models")
            if r.is_success and r.json().get("data"):
                api_model = r.json()["data"][0]["id"]
                print(f"Using API model id: {api_model}")
...
def main():
    ...
    parser.add_argument("--model", default="openai/gpt-oss-20b", help="Model name (for tokenizer and prompt)")
    parser.add_argument(
        "--question",
    ...
    ok = asyncio.run(test_tool_call(args.base_url, args.model, args.question))
